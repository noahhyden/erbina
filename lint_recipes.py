...
The actual rules live in `validate_recipe` in server.py — recipe LOADING and this
linter share one implementation, so a recipe that lints clean is exactly one
`bootstrap` will accept. (We depend on fastmcp only because importing server.py
pulls it in at module top; importing does not start the server — `mcp.run()` is
guarded by `__main__`.)
"""
...
from server import RECIPES_DIR, validate_recipe
...
def lint_path(path: Path) -> list[str]:
    ...
    return validate_recipe(data, stem=path.stem)
